<template>
  <VendorLayout>
    <div class="max-w-7xl mx-auto px-4 py-8">
      ...
      <!-- Pagination -->
      <div class="mt-4 flex justify-between items-center" v-if="pagination.pages > 1">
        <button
      ...
      <!-- Order Details Modal -->
      <div v-if="showOrderModal" class="fixed z-10 inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
          <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true" @click="closeOrderModal"></div>
                ...
                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                  ...
                  <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <p class="mt-1 text-sm text-gray-900">{{ selectedOrder.user?.name }}</p>
                      <p class="text-sm text-gray-500">{{ selectedOrder.user?.email }}</p>
                      <p class="text-sm text-gray-500">{{ selectedOrder.user?.phone }}</p>
                    </div>
                    <div>
                      <h4 class="text-sm font-medium text-gray-500">Shipping Address</h4>
                      <p class="mt-1 text-sm text-gray-900">{{ selectedOrder.shippingAddress?.addressLine1 }}</p>
                      <p class="text-sm text-gray-900">{{ selectedOrder.shippingAddress?.city }}, {{ selectedOrder.shippingAddress?.state }}</p>
                      <p class="text-sm text-gray-900">{{ selectedOrder.shippingAddress?.country }}</p>
                    </div>
                  </div>
                      ...
                      <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                          <tr v-for="item in selectedOrder.items" :key="item._id">
                            <td class="px-4 py-3 whitespace-nowrap">
                              <div class="flex items-center">
                                <div class="h-10 w-10 flex-shrink-0">
                                  <img class="h-10 w-10 rounded object-cover" :src="getProductImage(item.product)" alt="" />
                                </div>
                                <div class="ml-4">
                            ...
                            <td class="px-4 py-3 text-right text-sm text-gray-500">{{ formatCurrency(item.price) }}</td>
                            <td class="px-4 py-3 text-right text-sm text-gray-900">{{ formatCurrency(item.price * item.quantity) }}</td>
                          </tr>
                        </tbody>
