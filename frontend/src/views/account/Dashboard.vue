  ...
  <CustomerLayout>
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-3xl font-bold text-orange-600 mt-1">{{ stats.pendingOrders }}</p>
            </div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-3xl font-bold text-pink-600 mt-1">{{ stats.wishlistItems }}</p>
            </div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-3xl font-bold text-green-600 mt-1">{{ formatCurrency(stats.totalSpent) }}</p>
            </div>
      ...
      <div class="bg-white rounded-lg shadow-sm overflow-hidden">
        <div class="p-6 border-b border-gray-200">
          <div class="flex items-center justify-between">
            ...
            <router-link to="/account/orders" class="text-blue-600 hover:text-blue-700 font-medium text-sm">
              View All →
            </router-link>
          </div>
        </div>
        ...
        <div v-else class="divide-y divide-gray-200">
          <div v-for="order in recentOrders" :key="order._id" class="p-6 hover:bg-gray-50 transition-colors cursor-pointer" @click="goToOrder(order._id)">
            <div class="flex items-center justify-between">
              <div class="flex-1">
                <div class="flex items-center space-x-3">
                  <p class="font-semibold text-gray-900">Order #{{ order._id.slice(-8).toUpperCase() }}</p>
                  <span class="px-3 py-1 text-xs font-medium rounded-full" :class="getStatusClass(order.status)">
                    {{ order.status }}
                  </span>
                </div>
                <p class="text-sm text-gray-500 mt-1">{{ formatDate(order.createdAt) }}</p>
                <p class="text-sm text-gray-600 mt-1">{{ order.items.length }} item(s)</p>
              </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <router-link to="/account/profile" class="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition-shadow">
        ...
        <router-link to="/account/addresses" class="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition-shadow">
          <div class="flex items-center space-x-4">
            <div class="h-12 w-12 bg-green-100 rounded-lg flex items-center justify-center">
              <svg class="h-6 w-6 text-green-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <router-link to="/account/wishlist" class="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition-shadow">
          <div class="flex items-center space-x-4">
            <div class="h-12 w-12 bg-pink-100 rounded-lg flex items-center justify-center">
              <svg class="h-6 w-6 text-pink-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
...
const fetchDashboardData = async () => {
  ...
  try {
    ...
    stats.value.totalSpent = allOrders.reduce((sum, o) => sum + (o.total || 0), 0);
